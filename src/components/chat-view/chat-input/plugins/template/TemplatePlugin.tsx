...
import { TemplateDocument } from '../../../../../database/json/schemas/template'
...
class TemplateTypeaheadOption extends MenuOption {
  ...
  constructor(name: string, template: TemplateDocument) {
    super(name)
    this.name = name
...
export default function TemplatePlugin() {
  ...
  const [searchResults, setSearchResults] = useState<TemplateDocument[]>([])
  ...
  const handleDelete = useCallback(
    async (option: TemplateTypeaheadOption) => {
      await templateManager.deleteDocument(option.template.id)
      if (queryString !== null) {
