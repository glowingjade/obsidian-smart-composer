  ...
  advLineChanges.forEach((change: LineRangeMapping) => {
    ...
    if (oStart > lastOriginalEndLineNumberExclusive) {
      ...
      if (unchangedLines.length > 0) {
        blocks.push({
          ...
          lineCount: unchangedLines.length,
        })
      }
    }
    ...
    if (originalLines.length > 0 || modifiedLines.length > 0) {
      ...
      blocks.push({
        ...
        originalLineCount: originalLines.length > 0 ? originalLines.length : 0,
        modifiedLineCount: modifiedLines.length > 0 ? modifiedLines.length : 0,
        originalTokens:
          inlineTokens && originalLines.length > 0
            ? inlineTokens.originalTokens
            : undefined,
        modifiedTokens:
          inlineTokens && modifiedLines.length > 0
