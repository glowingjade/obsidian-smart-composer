...
class FileSuggestModal extends SuggestModal<TFile> {
  ...
  getSuggestions(query: string): TFile[] {
    ...
    return this.files.filter(file => 
      ...
      const aStartsWith = a.path.toLowerCase().startsWith(lowerCaseQuery) ? 0 : 1
      const bStartsWith = b.path.toLowerCase().startsWith(lowerCaseQuery) ? 0 : 1
...
export class CreateChatModal extends Modal {
  ...
  private onSubmit: (result: { message: string; file?: TFile | null } | null) => void

  constructor(app: App, onSubmit: (result: { message: string; file?: TFile | null } | null) => void) {
    super(app)
