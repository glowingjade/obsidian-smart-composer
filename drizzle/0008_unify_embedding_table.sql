...
CREATE INDEX "embeddings_dimensions_index" ON "embeddings" USING btree ("dimensions");--> statement-breakpoint
CREATE INDEX "embeddings_embedding_768_index" ON "embeddings" USING hnsw ((embedding::vector(768)) vector_cosine_ops) WHERE dimensions = 768;--> statement-breakpoint
CREATE INDEX "embeddings_embedding_1024_index" ON "embeddings" USING hnsw ((embedding::vector(1024)) vector_cosine_ops) WHERE dimensions = 1024;--> statement-breakpoint
CREATE INDEX "embeddings_embedding_1536_index" ON "embeddings" USING hnsw ((embedding::vector(1536)) vector_cosine_ops) WHERE dimensions = 1536;--> statement-breakpoint

-- Migrate data from openai-text-embedding-3-small
INSERT INTO "embeddings" ("path", "mtime", "content", "model", "dimensions", "embedding", "metadata")
